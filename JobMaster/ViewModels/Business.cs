...
namespace JobMaster.ViewModels
{
    public class NettyBusiness
    {
        ...
        public async Task InitRequestAsyncNetty()
        {
    ...
    public class Business
    {
        ...
        public Business(DlmsSettingsViewModel dlmsSettingsViewModel, SerialPortViewModel serialPortViewModel, FrontEndProcessorViewModel tcpServerViewModel)
        {
            ...
            switch (dlmsSettingsViewModel.PhysicalChanelType)
            {
                ...
                case PhysicalChanelType.FrontEndProcess: LinkLayer = new NetLinkLayer(tcpServerViewModel.TcpServerHelper, tcpServerViewModel.CurrentSocketClient); break;
                default:
                    break;
            }
        ...
        {
            var request = AppProtocolFactory.CreateGetRequestNormal(cosemAttributeDescriptorWithSelection);
            await LinkLayer.SendAsync(Protocol.BuildFinalSendData(InterfaceType, request));
        ...
        public Task<bool> Execute21ENegotiateAsync()
        {
            ...
            {
                var SerialPortLinkLayer = (SerialPortLinkLayer)LinkLayer;
                var keepAutoDataReceived = false;
                if (SerialPortLinkLayer.PortMaster.IsAutoDataReceived)
                {
                    keepAutoDataReceived = true;
                    SerialPortLinkLayer.PortMaster.IsAutoDataReceived = false;
                }

                SerialPortLinkLayer.BackupPortPara();
                SerialPortLinkLayer.Init21ESerialPort(300);
                ...
                if (array.Length != 0 && EModeParser.CheckServerFrameWisEquals2(array))
                {
                    ...
                    SerialPortLinkLayer.LoadBackupPortPara();//即之前如申请9600，调用有则为9600等参数进行接受
                                                             // LinkLayer.PortMaster.BaudRate = 9600; //需要修改波特率 ，再去接收
                    array = SerialPortLinkLayer.PortMaster.TryToReadReceiveData();
                    if (array.Length != 0 && EModeParser.CheckServerFrameZisEqualsClient(array))
                    {
                        SerialPortLinkLayer.PortMaster.SerialPortLogger.AddInfo("协商成功");
                        SerialPortLinkLayer.LoadBackupPortPara();
                        if (keepAutoDataReceived)
                        {
                            SerialPortLinkLayer.PortMaster.IsAutoDataReceived = true;
                        }
        ...
        {
            return await LinkLayer.SendAsync(Protocol.SetEnterUpGradeMode());
        }
    }
}