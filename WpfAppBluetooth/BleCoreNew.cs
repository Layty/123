    ...
    public class BleCoreNew
    {
        public class BluetoothLECode
        {
            ...
            public async Task SelectDevice(string MAC)
            {
                ...
                CurrentDevice = null;
                DeviceInformation.FindAllAsync(BluetoothLEDevice.GetDeviceSelector()).Completed =
                    async (asyncInfo, asyncStatus) =>
                    {
            ...
            public async Task SelectDeviceService()
            {
                ...
                CurrentDevice.GetGattServicesForUuidAsync(guid).Completed = async (asyncInfo, asyncStatus) =>
                {
                    if (asyncStatus == AsyncStatus.Completed)
                    {
                        try
