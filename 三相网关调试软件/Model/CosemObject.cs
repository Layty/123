...
namespace 三相智慧能源网关调试软件.Model
{
    public class CosemObjectViewModel : ObservableObject
    {
        public CosemObjectViewModel()
        {
            ...
            {
                var client = new RestClient("http://localhost:5000/api/CosemObjects");
            ...
            {
                var client = new RestClient($"http://localhost:5000/api/CosemObjects/ByObis/{s}");
                var request = new RestRequest(Method.GET);
            ...
            {
                var client = new RestClient($"http://localhost:5000/api/CosemObjects/ByClassId/{s}");
                var request = new RestRequest(Method.GET);
                ...
                try
                {
                    ...
                    CosemObjects  = JsonConvert.DeserializeObject<ObservableCollection<CosemObjectEdit>>(response.Content);
            ...
            {
                var client = new RestClient($"http://localhost:5000/api/CosemObjects/ByName/{s}");
                var request = new RestRequest(Method.GET);
                IRestResponse response = client.Execute(request);
                try
                {
            ...
            AddCommand = new RelayCommand(() =>
            {
                try
                {
                    ...
                    CurrentCosemObjectEdit.Id = new Guid();
            ...
            UpdateCommand = new RelayCommand<CosemObjectEdit>((t) =>
            {
                try
                {
                    var client = new RestClient($"http://localhost:5000/api/CosemObjects/{t.Obis}");
