    ...
    */
    public class IICInstantData
    {
        ...
        public bool  ParseData(string inputBytes)
        {
            try
            {
                ...
                if (bytes[0] == 0x80 && bytes[1] == 0x01)
                {
                    ...
                    P = ValueConvertor(bytes.Skip(36), ("W", 3),isUnsigned:false);
                    Pa = ValueConvertor(bytes.Skip(40), ("W", 3), isUnsigned: false);
                    Pb = ValueConvertor(bytes.Skip(44), ("W", 3), isUnsigned: false);
                    Pc = ValueConvertor(bytes.Skip(48), ("W", 3), isUnsigned: false);

                    Q = ValueConvertor(bytes.Skip(52), ("var", 3), isUnsigned: false);
                    Qa = ValueConvertor(bytes.Skip(56), ("var", 3), isUnsigned: false);
                    Qb = ValueConvertor(bytes.Skip(60), ("var", 3), isUnsigned: false);
                    Qc = ValueConvertor(bytes.Skip(64), ("var", 3), isUnsigned: false);
                    ...
                    Sc = ValueConvertor(bytes.Skip(80), ("VA", 3));
                     ...
                     Pf = ValueConvertor(bytes.Skip(84), ("", 4), isUnsigned: false);
                    Pfa = ValueConvertor(bytes.Skip(88), ("", 4), isUnsigned: false);
                    Pfb = ValueConvertor(bytes.Skip(92), ("", 4), isUnsigned: false);
                    Pfc = ValueConvertor(bytes.Skip(96), ("", 4), isUnsigned: false);
                    ...
                    ErrStatus = ValueConvertor(bytes.Skip(148), ("", 0), 2);
                    StealStatus = ValueConvertor(bytes.Skip(150), ("", 0), 4);
                    EMUStatus = ValueConvertor(bytes.Skip(154), ("", 0), 4);
                    RunStatus = ValueConvertor(bytes.Skip(158), ("", 0), 2);
                    return true;
                }
                else
                {
            ...
            int length = 4, bool isUnsigned = true)
        {
            ...
            resultstring= HandlerUnitScalar(result, (unitAndScalar));

            return resultstring;
            // return (result*0.001).ToString();
        }

        private string HandlerUnitScalar(dynamic result, (string Unit, int Scalar) unitAndScalar)
        {
